...
public class UtenteDAOImpl implements UtenteDAO {
	...
	public int howManyUtenteAreAdmin() {
		TypedQuery<Long> query = entityManager.createQuery(
				"select count(u.id) from Utente u join u.ruoli r where r.codice like 'ROLE_ADMIN'", Long.class);
	...
	public List<Utente> usersByMonthAndYear(int numeroMese, int numeroAnno) {
		...
		TypedQuery<Utente> query = entityManager
				.createQuery("FROM Utente u where month(u.dateCreated)=?1 and year(u.dateCreated)=?2", Utente.class);
